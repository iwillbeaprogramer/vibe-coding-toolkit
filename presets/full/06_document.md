...
8. `[내용]`, `[기능명]`, `path/to/file.py`, `tests/test_xxx.py`, `패키지명` 같은 템플릿 placeholder가 남아 있으면 실패로 처리한다.
...
with ZipFile(path) as zf:
    ...
    forbidden_placeholders = [
        "[내용]", "[기능명]", "path/to/file.py", "tests/test_xxx.py", "패키지명"
    ]
